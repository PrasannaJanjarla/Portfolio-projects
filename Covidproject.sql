...
SELECT
    ...
    CASE WHEN SUM(TRY_CAST(new_cases AS bigint)) > 0
         THEN (SUM(TRY_CAST(new_deaths AS bigint)) / SUM(TRY_CAST(new_cases AS bigint))) * 100
         ELSE 0
...
SELECT
    location,
    SUM(TRY_CAST(new_deaths AS INT)) AS TotalDeathCount
FROM
    PortfolioProject..CovidDeaths
WHERE
...
WHERE
  CAST(population_density AS FLOAT) > 0  -- Exclude rows with population_density = 0
GROUP BY
  location,date,population_density
ORDER BY
  HighestInfectedPeoplePercentage DESC;
